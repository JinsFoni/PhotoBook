...
STRINGS: dict[str, dict[str, str]] = {
    ...
    "从采集历史补录标签": {"zh-CN": "从采集历史补录标签", "zh-TW": "從採集歷史補錄標籤"},
    "重新解析已完成的采集任务详情页,把标签补到对应合集与模特。幂等,可重复执行。": {
        "zh-CN": "重新解析已完成的采集任务详情页,把标签补到对应合集与模特。幂等,可重复执行。",
        "zh-TW": "重新解析已完成的採集任務詳情頁,把標籤補到對應合集與模特。冪等,可重複執行。"},
    "补录完成:匹配 {m} 个,新关联 {n} 条,跳过 {k} 个": {
        "zh-CN": "补录完成:匹配 {m} 个,新关联 {n} 条,跳过 {k} 个",
        "zh-TW": "補錄完成:匹配 {m} 個,新關聯 {n} 條,跳過 {k} 個"},
}
